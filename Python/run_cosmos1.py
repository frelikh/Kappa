...
root1,root2 = n.loadtxt('COSMOS_all.list',dtype=int,unpack=True,usecols=(0,1))
nlens = root1.size
...
for j in range(4):
   f = open('Cosmos_counts/cosmos%d.txt' % j,'w')

   for i in range(nlens):
      catname = 'Cat_cosmos/%s'%str(root1[i]).zfill(6)+'+'+'%s'%str(root2[i]).zfill(4)+'_I.cat'
